...
export function PortalApp() {
  ...
  return <main className="min-h-screen bg-[#f3f7fc] text-[#172033]">
    ...
    : <section className="mx-auto max-w-6xl px-5 py-8">{loading ? <p className="rounded-2xl bg-white p-6 font-bold">Cargando tu cuenta…</p> : error ? <div className="rounded-2xl border border-red-200 bg-white p-6"><p className="font-bold text-red-700">{error}</p><button className="mt-4 text-sm font-bold text-[#075ac7]" onClick={() => client?.auth.signOut()} type="button">Salir</button></div> : summary ? <div className="grid gap-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><Link className="rounded-2xl bg-[#075ac7] p-6 text-white shadow-lg" href="/tienda"><strong className="text-xl">Armar un pedido</strong><span className="mt-2 block text-white/75">Elegí productos del catálogo y envialos a Starlim.</span></Link><button className="rounded-2xl border border-[#bcd5ef] bg-white p-6 text-left disabled:opacity-50" disabled={!sales.length || loading} onClick={repeatLastOrder} type="button"><strong className="text-xl">Repetir último pedido</strong><span className="mt-2 block text-[#64748b]">Crea una solicitud con los mismos artículos, cantidades y precios congelados.</span></button></div>
...
function PortalMetric({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl border border-[#dbe5f1] bg-white p-5 shadow-sm"><span className="text-xs font-extrabold uppercase tracking-[.08em] text-[#64748b]">{label}</span><strong className="mt-2 block text-2xl font-black">{value}</strong></div>; }
...
function PortalTable({ title, rows, empty }: { title: string; rows: string[][]; empty: string }) { return <section className="overflow-hidden rounded-2xl border border-[#dbe5f1] bg-white"><h2 className="border-b border-[#e4ebf3] px-5 py-4 text-xl font-black">{title}</h2>{rows.length ? <div className="divide-y divide-[#edf1f6]">{rows.slice(0, 20).map((row, index) => <div className="grid gap-1 px-5 py-4 sm:grid-cols-4" key={`${row[0]}-${index}`}>{row.map((cell, cellIndex) => <span className={cellIndex === 1 ? "font-bold" : "text-sm text-[#53657a]"} key={cellIndex}>{cell}</span>)}</div>)}</div> : <p className="p-5 text-[#64748b]">{empty}</p>}</section>; }
