...
export function PortalApp() {
  ...
  const checkPayment = useCallback(async () => {
    if (!paymentIntentId || !session) return;
      ...
      try {
        ...
        setCheckout((current) => current ? { ...current, status } : current); setError("");
        if (status === "approved") {
      ...
      } catch { setCheckout((current) => current ? { ...current, status: "pending" } : current); setError("No pudimos verificar el pago en este momento. El QR sigue vigente y volveremos a intentarlo automáticamente."); }
  ...
  return <main className="min-h-screen bg-[#f3f7fc] text-[#172033]">
    ...
    : <section className="mx-auto max-w-6xl px-5 py-8">{loading ? <p className="rounded-2xl bg-white p-6 font-bold">Cargando tu cuenta…</p> : !summary && error ? <div className="rounded-2xl border border-red-200 bg-white p-6"><p className="font-bold text-red-700">{error}</p><button className="mt-4 text-sm font-bold text-[#075ac7]" onClick={() => client?.auth.signOut()} type="button">Salir</button></div> : summary ? <div className="grid gap-6">
      ...
      {message ? <p className="rounded-2xl bg-emerald-50 p-4 font-bold text-emerald-700">{message}</p> : null}{error ? <p className="rounded-2xl border border-amber-200 bg-amber-50 p-4 font-bold text-amber-800">{error}</p> : null}
      <nav aria-label="Navegación del portal" className="sticky top-2 z-20 flex gap-2 overflow-x-auto rounded-2xl border border-[#dbe5f1] bg-white/95 p-2 shadow-md backdrop-blur">
...
function PortalTable({ title, rows, empty }: { title: string; rows: string[][]; empty: string }) {
  const [showAll, setShowAll] = useState(false);
  const [query, setQuery] = useState("");
  const filtered = rows.filter((row) => row.some((cell) => cell.toLocaleLowerCase("es").includes(query.trim().toLocaleLowerCase("es"))));
  const visible = filtered.slice(0, showAll ? filtered.length : 3);
  return <section className="overflow-hidden rounded-2xl border border-[#dbe5f1] bg-white shadow-sm">
    <RecordHeader count={filtered.length} title={title} />
    {rows.length > 3 ? <div className="border-b border-[#edf1f6] p-4"><input aria-label={`Buscar en ${title}`} className="min-h-11 w-full rounded-xl border border-[#cbd8e8] px-4 text-sm outline-none focus:border-[#075ac7]" onChange={(event) => { setQuery(event.target.value); setShowAll(false); }} placeholder="Buscar por fecha, número o estado" value={query} /></div> : null}
    {visible.length ? <div className="divide-y divide-[#edf1f6]">{visible.map((row, index) => <div className="grid gap-1 px-5 py-4 transition hover:bg-[#f8fbff] sm:grid-cols-4" key={`${row[0]}-${index}`}>{row.map((cell, cellIndex) => { const normalized = cell.toLocaleLowerCase("es"); const tone = normalized.includes("entregado") ? "font-bold text-emerald-600" : normalized.startsWith("cargo ") ? "font-bold text-red-600" : cellIndex === 1 ? "font-bold" : "text-sm text-[#53657a]"; return <span className={tone} key={cellIndex}>{cell}</span>; })}</div>)}{filtered.length > 3 ? <ShowMoreButton expanded={showAll} hiddenCount={filtered.length - 3} onClick={() => setShowAll((value) => !value)} /> : null}</div> : <p className="p-5 text-[#64748b]">{query ? "No encontramos registros con esa búsqueda." : empty}</p>}
  </section>;
}

function PortalDocumentTable({ title, rows, empty, onOpen }: { title: string; rows: { id: string; cells: string[] }[]; empty: string; onOpen: (id: string) => void }) {
  const [showAll, setShowAll] = useState(false);
  const [query, setQuery] = useState("");
  const filtered = rows.filter((row) => row.cells.some((cell) => cell.toLocaleLowerCase("es").includes(query.trim().toLocaleLowerCase("es"))));
  const visible = filtered.slice(0, showAll ? filtered.length : 3);
  return <section className="overflow-hidden rounded-2xl border border-[#dbe5f1] bg-white shadow-sm">
    <RecordHeader count={filtered.length} title={title} />
    {rows.length > 3 ? <div className="border-b border-[#edf1f6] p-4"><input aria-label={`Buscar en ${title}`} className="min-h-11 w-full rounded-xl border border-[#cbd8e8] px-4 text-sm outline-none focus:border-[#075ac7]" onChange={(event) => { setQuery(event.target.value); setShowAll(false); }} placeholder="Buscar por fecha, número o estado" value={query} /></div> : null}
    {visible.length ? <div className="divide-y divide-[#edf1f6]">{visible.map((row) => <div className="grid items-center gap-x-5 gap-y-3 px-5 py-4 transition hover:bg-[#f8fbff] sm:grid-cols-[minmax(110px,.8fr)_minmax(220px,1.5fr)_minmax(100px,.7fr)_minmax(120px,.8fr)_auto]" key={row.id}>{row.cells.map((cell, index) => <span className={`${index === 1 ? "font-bold" : cell.toLowerCase().includes("entregado") ? "font-bold text-emerald-600" : "text-sm text-[#53657a]"} ${index === row.cells.length - 1 ? "sm:text-right" : ""}`} key={index}>{cell}</span>)}<button className="min-h-10 rounded-lg border border-[#bcd5ef] px-4 text-sm font-bold text-[#075ac7] transition hover:border-[#075ac7] hover:bg-[#eef6ff]" onClick={() => onOpen(row.id)} type="button">Ver PDF</button></div>)}{filtered.length > 3 ? <ShowMoreButton expanded={showAll} hiddenCount={filtered.length - 3} onClick={() => setShowAll((value) => !value)} /> : null}</div> : <p className="p-5 text-[#64748b]">{query ? "No encontramos registros con esa búsqueda." : empty}</p>}
  </section>;
}
