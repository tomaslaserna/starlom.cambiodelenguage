...
function PaymentCheckout({ sales, selected, checkout, loading, onToggle, onCreate }: {
  ...
  return <section className="scroll-mt-5 overflow-hidden rounded-2xl border border-[#bcd5ef] bg-white shadow-sm" id="pagar">
    ...
    {sales.length ? <div className="divide-y divide-[#edf1f6]">{sales.map((sale) => <label className="grid cursor-pointer items-center gap-3 px-5 py-4 hover:bg-[#f7faff] sm:grid-cols-[auto_1.2fr_1fr_1fr_auto]" key={sale.id}><input checked={selected.includes(sale.id)} className="size-5 accent-[#075ac7]" onChange={() => onToggle(sale.id)} type="checkbox" /><strong>{sale.invoice_number ? `Factura ${sale.invoice_number}` : sale.number || "Pedido"}</strong><span className="text-sm text-[#64748b]">{sale.date}</span><span className="text-sm text-[#64748b]">{sale.invoice_number ? `Pedido ${sale.number}` : "Comprobante pendiente"}</span><strong className="text-red-600">{money.format(Number(sale.outstanding))}</strong></label>)}</div> : <p className="p-5 font-bold text-emerald-700">No tenés comprobantes pendientes para esta sucursal.</p>}
