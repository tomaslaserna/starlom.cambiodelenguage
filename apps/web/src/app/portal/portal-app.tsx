...
function PortalDocumentTable({ title, rows, empty, onOpen }: { title: string; rows: { id: string; cells: string[] }[]; empty: string; onOpen: (id: string) => void }) { const [showAll, setShowAll] = useState(false); return <section className="overflow-hidden rounded-2xl border border-[#dbe5f1] bg-white shadow-sm"><RecordHeader count={rows.length} title={title} />{rows.length ? <div className="divide-y divide-[#edf1f6]">{rows.slice(0, showAll ? rows.length : 3).map((row) => <div className="grid items-center gap-2 px-5 py-4 transition hover:bg-[#f8fbff] sm:grid-cols-[1fr_1.4fr_1fr_auto]" key={row.id}>{row.cells.map((cell, index) => <span className={index === 1 ? "font-bold" : cell.toLowerCase().includes("entregado") ? "font-bold text-emerald-600" : "text-sm text-[#53657a]"} key={index}>{cell}</span>)}<button className="rounded-lg border border-[#bcd5ef] px-3 py-2 text-sm font-bold text-[#075ac7] transition hover:border-[#075ac7] hover:bg-[#eef6ff]" onClick={() => onOpen(row.id)} type="button">Ver PDF</button></div>)}{rows.length > 3 ? <ShowMoreButton expanded={showAll} hiddenCount={rows.length - 3} onClick={() => setShowAll((value) => !value)} /> : null}</div> : <p className="p-5 text-[#64748b]">{empty}</p>}</section>; }
