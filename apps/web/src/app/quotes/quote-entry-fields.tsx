...
export function QuoteEntryFields({ clients, products }: QuoteEntryFieldsProps) {
  ...
  const canSendQuickQuote = Boolean(selectedClient && calculatedLines.length && totalAmount > 0);
  const quickQuoteText = selectedClient
    ? [
        `Hola ${selectedClient.name}, te paso presupuesto rapido de Starlim:`,
        "",
        ...calculatedLines.map(
          (line) =>
    ...
    : "";
  const quickQuotePhone = selectedClient ? whatsappPhone(selectedClient.phone) : "";
    ...
    <div className="grid gap-4">
      ...
      {selectedClient ? (
        <div className="grid gap-3 rounded-lg border border-[color:var(--border)] bg-[color:var(--panel-subtle)] p-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Field htmlFor="quote-price-list" label="Lista">
            <Select
              id="quote-price-list"
      ...
      <div className="flex flex-col justify-end gap-2 sm:flex-row">
        ...
        <Button type="submit">Crear presupuesto formal</Button>
      </div>
    </div>
  );
}
