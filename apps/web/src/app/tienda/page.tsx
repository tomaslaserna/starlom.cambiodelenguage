...
export default async function StorePage() {
  const catalog = await listSalePrices({ companyId: 1, pageSize: "200" });
  const products = catalog.data.map(({ id, name, code, category, supplier, imageUrl }) => ({ id, name, code, category, brand: supplier, imageUrl }));
