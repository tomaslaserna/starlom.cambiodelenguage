...
export function Storefront({ products, recommendations = [], combos = [], portalClientId = "" }: { products: Product[]; recommendations?: SegmentRecommendation[]; combos?: Combo[]; portalClientId?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/storefront/requests", { method: "POST", headers: { "Content-Type": "application/json", ...(accessToken ? { authorization: `Bearer ${accessToken}` } : {}) }, body: JSON.stringify({ ...Object.fromEntries(form.entries()), ...discovery, usualPurchases: discovery.usualPurchases, ...location, portalClientId, requestKey: requestKeyRef.current, challengeStartedAt: challengeStartedAt ? new Date(challengeStartedAt).toISOString() : "", items: selected.map((product) => ({ productId: product.id, quantity: cart[product.id] })) }) });
      const payload = await response.json();
  ...
  if (step === "success") return <section className="mx-auto grid min-h-[520px] max-w-3xl place-items-center px-5 py-16 text-center"><div className="rounded-[22px] border border-[#b8e3cf] bg-white p-8 shadow-xl sm:p-12"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-[#e7f8ef] text-3xl text-[#07834f]">✓</span><h2 className="mt-6 text-3xl font-extrabold">Solicitud recibida</h2>{submittedReference && <p className="mt-3 text-xl font-extrabold text-[#075ac7]">Referencia {submittedReference}</p>}<p className="mt-4 text-lg leading-7 text-[#5b6b82]">Ya ingresó al CRM de Starlim y quedó asignada para revisión comercial. Conservá esta referencia; nos comunicaremos para confirmar disponibilidad, IVA y entrega.</p><div className="mt-5 grid gap-2 rounded-xl bg-[#eef5ff] p-4 text-left text-sm font-semibold text-[#315170]"><span>✓ Carrito y precios registrados</span><span>✓ Prospecto creado para seguimiento</span><span>Próximo paso: confirmación de un comercial</span></div><button className="mt-8 rounded-[11px] bg-[#075ac7] px-6 py-3 font-bold text-white" onClick={() => { setCart({}); requestKeyRef.current = ""; setSubmittedReference(""); setStep("catalog"); }} type="button">Volver a la tienda</button></div></section>;
