...
type StorePayment = { intentId: string; requestKey: string; amount: number; checkoutUrl: string; qrDataUrl: string; status: string; orderNumber?: string };
...
export function Storefront({ products, recommendations = [], combos = [], portalClientId = "" }: { products: Product[]; recommendations?: SegmentRecommendation[]; combos?: Combo[]; portalClientId?: string }) {
  ...
  const [step, setStep] = useState<"catalog" | "checkout" | "payment" | "success">("catalog");
  ...
  const [submittedChallengeOrder, setSubmittedChallengeOrder] = useState("");
  ...
  const effectiveUnitPrice = (product: Product) => {
    ...
    return paymentMethod === "qr" || completesPresentation ? product.prices.list1 : qualifiesForList2 ? product.prices.list2 : product.prices.list3;
  };
  ...
  const vatAmount = Math.round(estimatedCartAmount * 0.21 * 100) / 100;
  const finalCartAmount = estimatedCartAmount + vatAmount;
  ...
  function changeQuantity(id: string, delta: number) {
    if (delta > 0 && products.find((product) => product.id === id)?.available === "out") return;
  ...
  function addCombo(combo: Combo) {
    setCart((current) => {
      ...
      for (const item of combo.items) {
        ...
        if (product && product.available !== "out") next[item.productId] = Math.max(next[item.productId] ?? 0, item.quantity);
      }
      return next;
    });
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSubmittedChallengeOrder(String(payload.data?.challengeOrder?.orderNumber ?? ""));
      if (payload.data?.payment) { setStorePayment({ ...payload.data.payment, status: "created" }); setStep("payment"); }
      else setStep("success"); window.scrollTo({ top: 0, behavior: "smooth" });
    } catch (cause) { setError(cause instanceof Error ? cause.message : "No pudimos enviar el pedido"); }
    finally { setSubmitting(false); }
  }

  useEffect(() => {
    if (step !== "payment" || !storePayment || ["approved", "rejected", "cancelled"].includes(storePayment.status)) return;
    const poll = async () => {
      const response = await fetch(`/api/storefront/payments/${storePayment.intentId}/status?key=${storePayment.requestKey}`, { cache: "no-store" });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) return;
      const next = { ...storePayment, ...payload.data };
      setStorePayment(next);
      if (next.status === "approved") window.setTimeout(() => setStep("success"), 900);
    };
    void poll();
  ...
  if (step === "payment" && storePayment) return <section className="mx-auto grid min-h-[620px] max-w-3xl place-items-center px-5 py-12 text-center"><div className="w-full rounded-[24px] border border-[#b9d3ef] bg-white p-7 shadow-xl sm:p-10"><span className="text-xs font-black uppercase tracking-[.14em] text-[#075ac7]">Pago seguro</span><h2 className="mt-2 text-3xl font-black">Escaneá el QR de Mercado Pago</h2><p className="mt-2 text-[#64748b]">Referencia {submittedReference} · Total final con IVA 21%</p><strong className="mt-4 block text-3xl text-[#075ac7]">{new Intl.NumberFormat("es-AR", { style: "currency", currency: "ARS" }).format(storePayment.amount)}</strong><Image alt="QR de Mercado Pago" className="mx-auto mt-5 rounded-2xl border border-[#dbe5f1]" height={320} src={storePayment.qrDataUrl} unoptimized width={320} /><a className="mt-5 inline-flex rounded-xl bg-[#075ac7] px-6 py-3 font-black text-white" href={storePayment.checkoutUrl} rel="noreferrer" target="_blank">Abrir Mercado Pago</a><div className="mt-6 rounded-xl bg-[#eef5ff] p-4 text-left text-sm font-semibold text-[#315170]"><span className="block">{storePayment.status === "approved" ? "✓ Pago acreditado" : "1. Esperando acreditación de Mercado Pago…"}</span><span className="mt-2 block">2. Al acreditarse, el pedido entra automáticamente al ERP.</span><span className="mt-2 block">3. Recibirás la confirmación de entrega.</span></div></div></section>;

  if (step === "success") return <section className="mx-auto grid min-h-[520px] max-w-3xl place-items-center px-5 py-16 text-center"><div className="rounded-[22px] border border-[#b8e3cf] bg-white p-8 shadow-xl sm:p-12"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-[#e7f8ef] text-3xl text-[#07834f]">✓</span><h2 className="mt-6 text-3xl font-extrabold">Solicitud recibida</h2>{submittedReference && <p className="mt-3 text-xl font-extrabold text-[#075ac7]">Referencia {submittedReference}</p>}<p className="mt-4 text-lg leading-7 text-[#5b6b82]">{paymentMethod === "qr" ? "El pago fue acreditado y el pedido ingresó automáticamente al ERP, pendiente de entrega." : submittedChallengeOrder ? `Completaste el Desafío Starlim: el pedido ${submittedChallengeOrder} ingresó al ERP y quedó pendiente de confirmación.` : "La solicitud ingresó al CRM de Starlim y quedó asignada para confirmación comercial."}</p><div className="mt-5 grid gap-2 rounded-xl bg-[#eef5ff] p-4 text-left text-sm font-semibold text-[#315170]">{paymentMethod === "qr" ? <><span>✓ Pago acreditado</span><span>✓ Pedido registrado en el ERP</span><span>Próximo paso: preparación y entrega</span></> : submittedChallengeOrder ? <><span>✓ Ubicación, tiempo y compra mínima validados</span><span>✓ Pedido registrado en el ERP</span><span>Próximo paso: confirmación de Starlim; desde allí corre la garantía de 24 horas hábiles</span></> : <><span>✓ Carrito y precios registrados</span><span>✓ Prospecto creado para seguimiento</span><span>Próximo paso: confirmación de un comercial</span></>}</div><button className="mt-8 rounded-[11px] bg-[#075ac7] px-6 py-3 font-bold text-white" onClick={() => { setCart({}); requestKeyRef.current = ""; setSubmittedReference(""); setSubmittedChallengeOrder(""); setStorePayment(null); setPaymentMethod("cash"); setStep("catalog"); }} type="button">Volver a la tienda</button></div></section>;
  ...
  return <section className={`${challengeActive ? "challenge-fire-screen " : ""}mx-auto max-w-[1380px] px-4 py-8 sm:px-8`}>
    {step === "catalog" ? <>
      ...
      <div className="rounded-[18px] border border-[#dbe5f1] bg-white p-5 shadow-sm sm:p-7"><button className="mb-5 text-sm font-bold text-[#075ac7]" onClick={() => setStep("catalog")} type="button">← Volver al catálogo</button><span className="text-xs font-extrabold uppercase tracking-[0.12em] text-[#075ac7]">Último paso</span><h2 className="mt-2 text-2xl font-extrabold">¿A dónde te enviamos la propuesta?</h2><p className="mt-2 text-[#64748b]">Ya elegiste lo que te interesa. Ahora un comercial prepara la cotización y te contacta.</p><div className="mt-6 grid gap-4 sm:grid-cols-2">{[["name","Nombre y apellido *"],["phone","WhatsApp o teléfono *"],["email","Email"],["brand","Marca o nombre comercial"],["taxId","CUIT"],["businessName","Razón social"]].map(([name,label]) => <label className="grid gap-1.5 text-sm font-bold" key={name}>{label}<input className={fieldClass} defaultValue={name === "name" ? challengeCustomer.name : name === "phone" ? challengeCustomer.phone : name === "businessName" ? challengeCustomer.businessName : ""} name={name} type={name === "email" ? "email" : "text"} required={name === "name" || name === "phone" || (name === "email" && paymentMethod === "qr")} /></label>)}<label className="grid gap-1.5 text-sm font-bold">Rubro<input className={fieldClass} name="industry" onChange={(event) => setDiscovery((current) => ({ ...current, industry: event.target.value }))} value={discovery.industry} /></label></div>
      <fieldset className="mt-6"><legend className="text-sm font-black">Forma de pago</legend><div className="mt-2 grid gap-3 sm:grid-cols-2"><button className={`rounded-xl border p-4 text-left ${paymentMethod === "cash" ? "border-[#075ac7] bg-[#eef5ff] ring-2 ring-[#075ac7]/20" : "border-[#dbe5f1]"}`} onClick={() => setPaymentMethod("cash")} type="button"><strong className="block">Efectivo al recibir</strong><span className="mt-1 block text-sm text-[#64748b]">El pedido queda para confirmación comercial.</span></button><button className={`rounded-xl border p-4 text-left ${paymentMethod === "qr" ? "border-[#075ac7] bg-[#eef5ff] ring-2 ring-[#075ac7]/20" : "border-[#dbe5f1]"}`} onClick={() => setPaymentMethod("qr")} type="button"><strong className="block">QR Mercado Pago</strong><span className="mt-1 block text-sm text-[#64748b]">Precio Lista 1 y acreditación en vivo.</span></button></div><p className="mt-2 text-xs font-semibold text-[#64748b]">La cuenta corriente es exclusiva para clientes habilitados en el Portal de clientes.</p></fieldset>
      ...
      <aside className="h-fit rounded-[18px] border border-[#dbe5f1] bg-white p-5 shadow-sm lg:sticky lg:top-24"><h2 className="text-xl font-extrabold">Tu carrito</h2><div className="mt-4 divide-y divide-[#e5ebf2]">{selected.map((product) => <div className="flex items-center justify-between gap-4 py-3" key={product.id}><span className="font-semibold">{product.name}</span><strong className="shrink-0">× {cart[product.id]}</strong></div>)}</div><div className="mt-4 rounded-[12px] bg-[#eef5ff] p-4 text-sm text-[#315170]"><div className="flex justify-between gap-3"><span>Precio aplicado</span><strong>{paymentMethod === "qr" ? "Lista 1 por pago inmediato" : qualifiesForList2 ? "Lista 2" : "Lista 3"}</strong></div><div className="mt-2 flex justify-between gap-3"><span>Subtotal neto</span><strong>{new Intl.NumberFormat("es-AR", { style: "currency", currency: "ARS" }).format(estimatedCartAmount)}</strong></div><div className="mt-2 flex justify-between gap-3"><span>IVA 21%</span><strong>{new Intl.NumberFormat("es-AR", { style: "currency", currency: "ARS" }).format(vatAmount)}</strong></div><div className="mt-3 flex justify-between gap-3 border-t border-[#c9d9eb] pt-3 text-lg"><span>Total final</span><strong>{new Intl.NumberFormat("es-AR", { style: "currency", currency: "ARS" }).format(finalCartAmount)}</strong></div>{estimatedSavings > 0 && <p className="mt-2 font-extrabold text-emerald-700">Ahorrás {new Intl.NumberFormat("es-AR", { style: "currency", currency: "ARS" }).format(estimatedSavings)}</p>}{!qualifiesForList2 && <p className="mt-2 font-semibold">Sumá {new Intl.NumberFormat("es-AR", { style: "currency", currency: "ARS" }).format(Math.max(0, 50_000 - list3Amount))} para acceder a Lista 2.</p>}<p className="mt-2 text-xs">Los artículos con bulto completo usan Lista 1. Todos los pedidos web se emiten con IVA 21%.</p></div>{challengeStartedAt ? <p className={`mt-3 rounded-[10px] p-3 text-sm font-bold ${challengeActive && challengeMinimumReached ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-800"}`}>{!challengeActive ? "El temporizador del desafío venció." : challengeMinimumReached ? "Tu solicitud cumple el mínimo del Desafío Starlim." : "Sumá productos hasta alcanzar el mínimo interno de $150.000."}</p> : null}<button className="mt-5 w-full rounded-[11px] bg-[#075ac7] px-5 py-3.5 font-extrabold text-white disabled:opacity-50" disabled={submitting || Boolean(challengeStartedAt && (!challengeActive || !challengeMinimumReached))} type="submit">{submitting ? "Procesando…" : paymentMethod === "qr" ? "Generar QR y pagar" : "Enviar pedido"}</button></aside>
    </form>}
  </section>;
}
