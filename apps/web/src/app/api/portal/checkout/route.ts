...
import { selectionIsOldestFirst } from "@/lib/portal-balances";
  ...
  try {
    ...
    const result = await withCompanyContext(identity.companyId, async (client) => {
      const rows = await client.query<{ id: string; client_id: string; date: string; outstanding: string }>(`
        SELECT s.id::text, s.client_id::text, s.sale_date::text AS date, GREATEST(COALESCE(m.debits,0)-COALESCE(m.credits,0),0)::text AS outstanding
      ...
      const openSales = rows.rows.filter((sale) => Number(sale.outstanding)>0.005);
