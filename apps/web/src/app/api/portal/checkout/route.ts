...
export async function POST(request: Request) {
  try {
    ...
    const saleIds = Array.from(new Set(Array.isArray(body.saleIds) ? body.saleIds.map(String) : []));
    ...
    const result = await withCompanyContext(identity.companyId, async (client) => {
      const rows = await client.query<{ id: string; outstanding: string }>(`
        SELECT s.id::text, GREATEST(COALESCE(s.total_amount,0)+COALESCE(m.debits,0)-COALESCE(m.credits,0),0)::text AS outstanding
        ...
        WHERE s.empresa_id=$1 AND s.client_id=$2::uuid AND s.id=ANY($3::uuid[])
          AND COALESCE(s.order_status,s.status,'')='entregado'
        FOR UPDATE OF s`, [identity.companyId, clientId, saleIds]);
      if (rows.rows.length !== saleIds.length) throw new ApiError(409, "Una de las facturas seleccionadas ya no está disponible");
      const amount = Math.round(rows.rows.reduce((sum, row) => sum + Number(row.outstanding), 0) * 100) / 100;
