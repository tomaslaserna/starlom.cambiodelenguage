...
export async function GET(
  ...
  try {
    ...
    const allowed = await queryWithCompanyContext(
      identity.companyId,
      `SELECT 1 FROM sales WHERE empresa_id=$1 AND id=$2::uuid AND client_id=ANY($3::uuid[])`,
      [identity.companyId, id, identity.clientIds],
      { cache: false },
    );
    if (!allowed.rows.length)
      throw new Error("El pedido no pertenece a tu cuenta");
    return pdfResponse(
      await buildOrderRemitoPdf(identity.companyId, id, {
        includePrices: true,
      }),
