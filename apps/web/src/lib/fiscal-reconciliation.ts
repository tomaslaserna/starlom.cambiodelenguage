...
export async function linkIncidentInvoice(session: AuthSession, invoiceNumber: number) {
  ...
  await withCompanyContext(session.companyId, async (client) => {
    ...
    await client.query(`UPDATE sales SET fiscal_status='aprobado', fiscal_provider='arca', fiscal_mode='production', fiscal_document_source='sale', fiscal_document_kind='invoice', fiscal_point_of_sale=$1::integer, fiscal_receipt_type=$2::integer, fiscal_receipt_number=$3::integer, fiscal_vat_rate=$4::numeric, receipt_type=$2::integer, receipt_number=$3::bigint, cae=$5, cae_expires_at=$6::date, fiscal_issue_date=$7::date, fiscal_authorized_at=COALESCE(fiscal_authorized_at,now()), fiscal_last_attempt_at=now(), fiscal_error_code='', fiscal_error_message='', fiscal_observations=$8::jsonb, tracking_status='facturada', updated_at=now() WHERE id=$9::uuid AND empresa_id=$10`,
