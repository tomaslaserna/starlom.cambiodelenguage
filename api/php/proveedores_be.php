...
if ($accion === 'add_proveedor') {
    ...
    $s = $conexion->prepare(
        "INSERT INTO proveedores (nombre,contacto,telefono,email,direccion,notas) VALUES (?,?,?,?,?,?) RETURNING id"
    );
    $s->bind_param('ssssss', $nombre, $contacto, $telefono, $email, $direccion, $notas);
    $s->execute();
...
if ($accion === 'edit_proveedor') {
    ...
    $s->bind_param('ssssssi', $nombre, $contacto, $telefono, $email, $direccion, $notas, $id);
    $s->execute(); $s->close();
...
if ($accion === 'del_proveedor') {
    ...
    $s = $conexion->prepare("DELETE FROM proveedores WHERE id = ?");
